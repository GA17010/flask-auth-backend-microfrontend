...
def test_check_auth(client, new_user):
  ...
  response = client.get("/auth/checkAuth", headers={"Cookie": f"accessToken={access_token}"})
  assert response.status_code == 200
...
def test_refresh_token(client, new_user):
  ...
  login_response = client.post("/auth/login", json=login_data)
  print(login_response.headers.get("Set-Cookie"))
...
def test_logout_user(client, new_user):
  ...
  assert response.status_code == 200
  assert response.json["data"]["mensaje"] == "Seccion Cerrada"
